...
*/const student = {
  ...
  set gpa(v) {
    if (typeof v !== "number" || v < 0 || v > 4) throw new Error("Invalid GPA");
    this._gpa = parseFloat(v.toFixed(2));
  }
};
...
const pattern = /ab/;
const matched = [];
words.forEach(w => { if (pattern.test(w)) { console.log(`${w} matches!`); matched.push(w); } });
